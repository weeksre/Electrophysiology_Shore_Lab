function [betahat betaci]=cox(A,tsp,delta,tau,alphas,alphar,pval)
...
tspa=reshape(A,1,[]); % target spike train 
...
for j=1:p
    rand('twister',5555 );
    s=['m' int2str(j) '=randperm(size(isiat,2));'];
    eval(s);
...
%    poolobj=parpool(2); 
% end
for m=1:p % distributing the calculations of each neuron over the worker pool 
        t0=tic;
...
z = zeros (laf); 
   ...
   tt=toc(t0);
   fprintf('Z %d of %d calculated: %.2f seconds.\n',m,p,tt);
end;

bet=.2*ones(1,p)'; % initialization of beta values 
try
scc_all = zeros(p,laf,laf);
% 3D matrix resulted from summation of diagonal values of Z multiplied by corresponding betahat for calculating the loglikelihood
landa=1 ; 
for i=1:20 % starting the newton-raphson iterations 
    ...
    % work faster on GPUs, implicit multithreading:
%     [mm,jj] = meshgrid(1:p,1:p);
%     func_handle = @(mm,jj,ssum,z_all,sumte) sum(sum(tril(z_all{m}.*z_all{j}.*exp(ssum)))./sumte)-sum(sum(tril(z_all{m}.*exp(ssum))).*sum(tril(z_all{j}.*exp(ssum)))./sumte.^2);
%     
%     arrayfun 
    
    tt=toc(t0);
    fprintf('Hessian iteration #%d (max: 20): %.2f seconds.\n',i,tt);
    dot_tmp = infc'*infc;  
...
if (iteration_done_flag==0)
   ...
   disp(' Matrix size too big, please run again.')
    betahat=NaN;
    betaci=[NaN NaN] ;
end